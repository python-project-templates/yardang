...
```toml
[tool.yardang] extensions = ["your_package.sphinx"] html_theme_options = {} html_static_path = [] html_css_files = [] html_js_files = [] source_suffix = [] exclude_patterns = [] language = "en" pygments_style = "sphinx" 
```
...
```toml
[tool.yardang.llms] enabled = true description = "Documentation for My Project" full-build = true 
```
